...
void GraphCtrl::DrawRect()
{
	...
	//--------------------폰트설정----------------------//
	m_Yfont.CreateFont (14, 0, 0, 0, 100, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_SWISS, L"Arial") ;
	m_Xfont.CreateFont (14, 0, 0, 0, 100, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_SWISS, L"Arial") ;
	...
	//--------------------X축 데이터 값---------------------//
	// 150823 YJ remove count
	//CString str_width;					//데이터 갯수
	//str_width.Format(L"Data:%d",m_GraphWidth/m_XWidth);
	...
	for(int k=0; k<m_Num; k++)
	{
		...
		{
			CString str;
			str.Format(L"%.1f", m_sNameData[k].RangeMax -  m_sNameData[k].RangeAll/4*j);
